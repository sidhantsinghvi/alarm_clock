...
async def check_pins(dut):
    ...
    #clock pins
    assert len(dut.SECONDS.value) == 6, "SECONDS Pin"
    assert len(dut.MINUTES.value) == 6, "MINUTES Pin"
    assert len(dut.HOURS.value) == 5, "HOURS Pin"

    #alarm pins
    assert len(dut.ALARM_MINUTES.value) == 6, "ALARM_MINUTES Pin"
    assert len(dut.ALARM_HOURS.value) == 5, "ALARM_HOURS Pin"
